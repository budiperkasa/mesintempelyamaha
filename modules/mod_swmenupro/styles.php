...
function ClickMenuStyle($swmenupro,$ordered){
	...
	$str.=is_file($mosConfig_absolute_path."/".$swmenupro['main_back_image']) ? " background-image: URL(\"".$mosConfig_live_site."/".$swmenupro['main_back_image']."\") ;\n":"";
	...
	$str.=is_file($mosConfig_absolute_path."/".$swmenupro['main_back_image_over']) ? " background-image: URL(\"".$mosConfig_live_site."/".$swmenupro['main_back_image_over']."\") ;\n":"";
	...
	$str.=is_file($mosConfig_absolute_path."/".$swmenupro['sub_back_image']) ? " background-image: URL(\"".$mosConfig_live_site."/".$swmenupro['sub_back_image']."\") ;\n":"";
	...
	$str.=is_file($mosConfig_absolute_path."/".$swmenupro['sub_back_image_over']) ? " background-image: URL(\"".$mosConfig_live_site."/".$swmenupro['sub_back_image_over']."\") ;\n":"";
...
function cssTabMenuStyle($swmenupro,$ordered){
	...
	//$str.="#menu".$swmenupro['id']." li#here".$swmenupro['id']." ,\n";
	...
	for($i=0;$i<count($ordered);$i++){
		...
		if($ordered[$i]['OCSS']){
			$str.=(!$ordered[$i]['indent']?"#menu".$swmenupro['id']:"ul#submenu".$swmenupro['id'])." #swdyntab".$swmenupro['id'].$ordered[$i]['ID'].":hover {\n";
			$str.=$ordered[$i]['OCSS'];
			$str.="\n}\n"; 				
			}
	}
...
function dynamicTabMenuStyle($swmenupro,$ordered){
	global $mosConfig_live_site, $mosConfig_absolute_path;
...
$str.="}\n";
	...
	for($i=0;$i<count($ordered);$i++){
		...
		if($ordered[$i]['OCSS']){
			$str.=(!$ordered[$i]['indent']?"#menu".$swmenupro['id']:"table.submenu".$swmenupro['id'])." #swdyntab".$swmenupro['id'].$ordered[$i]['ID'].":hover {\n";
			$str.=$ordered[$i]['OCSS'];
			$str.="\n}\n"; 				
			}
	}
...
function mygosu_customcss($ordered,$item,$id,$class){
	...
	if($item['indent']==0){
		...
		if($class){
		$str.=".ddmx".$id." td.item11-acton-last #menu".$id.$k.$class." ,\n";
