...
function swGetMenuLinks($menu,$id,$hybrid,$use_tables){
	...
	if ($menu=="swcontentmenu") {
		...
		for($j=0;$j<count($result3);$j++) {
			...
			$swmenupro_array[] =array("TITLE" => $result4->product_name, "URL" =>  $url , "ID" => ($result4->product_id+100000)  , "PARENT" => ($result2->category_id?($result2->category_id+10000):0) ,  "ORDER" => $result2->list_order, "IMAGE" => $result4->image, "IMAGEOVER" => $result4->image_over, "SHOWNAME" => $result4->show_name, "IMAGEALIGN" => $result4->image_align, "TARGETLEVEL" => $result4->target_level, "TARGET" => 0,"ACCESS" => 0,"NCSS" => $result4->normal_css,"OCSS" => $result4->over_css,"SHOWITEM" => $result4->show_item  );
		}
		}
		}
...
$topmenu = chain('ID', 'PARENT', 'ORDER', $ordered, 0, 1);
	...
	if ($topmenu[$i]['TARGETLEVEL'] == "0"){
			...
			$str.=swislast($topmenu,$i)?"</td></tr></table>\n":"</td><td>\n";
	}
...
function doGosuMenu($ordered, $swmenupro, $active_menu, $css_load,$selectbox_hack,$padding_hack){
...
function doTransMenu($ordered, $swmenupro, $active_menu,  $sub_indicator, $parent_id, $css_load,$selectbox_hack,$show_shadow,$padding_hack){
...
function transMenu($ordered, $swmenupro, $active_menu,  $sub_indicator, $parent_id,$selectbox_hack){
	...
	if($sub_indicator){
		...
		$str.="TransMenu.dingbatOn = \"".$mosConfig_live_site."/modules/mod_swmenupro/images/transmenu/submenuleft-on.gif\";\n";
		$str.="TransMenu.dingbatOff = \"".$mosConfig_live_site."/modules/mod_swmenupro/images/transmenu/submenuleft-off.gif\"; \n";
		}else{
		$str.="TransMenu.dingbatOn = \"".$mosConfig_live_site."/modules/mod_swmenupro/images/transmenu/submenu-on.gif\";\n";
		$str.="TransMenu.dingbatOff = \"".$mosConfig_live_site."/modules/mod_swmenupro/images/transmenu/submenu-off.gif\"; \n";
		}
		...
		if ($ordered[$counter]['indent'] == 0){
			...
			while ($doSubMenu){
				if ($ordered[$counter]['indent'] != 0){
					...
					if (($counter+1 == $number) || swislast($ordered,$counter)){
