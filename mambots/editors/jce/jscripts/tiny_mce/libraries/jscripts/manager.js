...
function setIframeSrc(s){
	...
	getObj('manager').src = 'index2.php?option=com_jce&no_html=1&task=plugin&plugin='+jce.getPlugin()+'&token='+ token +'&file=files.php' + s;
};
function getDir(){
...
function createFileIFrame(dir, file) {
	...
	setImg('load.gif');	
    setHTML('fileContainer', '<iframe class="fileFrame" id="manager" name="manager" src="index2.php?option=com_jce&no_html=1&task=plugin&plugin='+ jce.getPlugin() +'&token='+ token +'&file=files.php' + dir + file + '" frameborder="0"></iframe>');
};
function setLoader(){
...
function setupIframe(s){
	var path = '', file = '', base = jce.get('base_url');
	
	s = s.replace(tinyMCE.getParam('document_base_url'), '', 'g');
	
	if(base.charAt(0) == '/' && s.charAt(0) != '/')
		s = '/' + s;

	if(s.length > 1)
		path = dirname(s).replace(base, '', 'g');
