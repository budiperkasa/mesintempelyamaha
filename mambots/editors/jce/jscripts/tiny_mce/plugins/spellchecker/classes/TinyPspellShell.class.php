...
class TinyPspellShell {
	...
	function TinyPspellShell(&$params, $lang, $mode, $spelling, $jargon, $encoding) {
		$this->lang = $lang;
		$this->mode = $mode;
		$this->error = false;
		$this->errorMsg = array();

		$this->tmpfile = tempnam($params->get('tinypspellshell_tmp'), "tinyspell");

		if(preg_match("#win#i",php_uname()))
            $this->cmd = $params->get('tinypspellshell_aspell') . " -a --lang=". $this->lang." --encoding=utf-8 -H < $this->tmpfile 2>&1";
        else
            $this->cmd = "cat ". $this->tmpfile ." | " . $params->get('tinypspellshell_aspell') . " -a --encoding=utf-8 -H --lang=". $this->lang;
	}
