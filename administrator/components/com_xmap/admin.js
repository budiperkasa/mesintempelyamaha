...
function saveSettings(sitemap) {
	showLoading();
	var theForm = $('frmSettings'+sitemap);
	var postVars = 'option=com_xmap&task=ajax_request&action=save_sitemap_settings&' + theForm.toQueryString();
				var myAjax = new Ajax(
				ajaxURL,
				{
						method: 'post',
						postBody: postVars,
						onComplete: checkResultSaveSettings.bindAsEventListener(this,[sitemap])
				}).request();
}
...
function checkResultSaveSettings(ajaxResponse,sitemapid) {
	hideLoading();
	if (ajaxResponse != '1') {
		alert(ajaxResponse);
	}else{
		closeSettings('sitemapsettings');
	}
}
