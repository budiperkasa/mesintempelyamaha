<meta http-equiv="content-type" content="text/html;charset=utf-8">
<?php
/**
* swmenupro v4.5
* http://swonline.biz
* Copyright 2006 Sean White
* German Translation by Stefan Mueller, milchi@milchi.de
**/

// no direct access
defined( '_VALID_MOS' ) or die( 'Restricted access' );


//swMenuPro 5.3 new terms

define( '_SW_COPY_IMAGES', 'Also copy custom images & CSS' );

define( '_SW_CLICK_TRANS_MENU', 'Click-Trans Menu' );

define( '_SW_PADDING_HACK', 'IE Size/Padding Hack' );

define( '_SW_PADDING_HACK_TIP', 'If you have menu item size differences between Internet Explorer and other browsers then try this hack.' );


//swMenuPro 5.0 new terms

define( '_SW_SLIDECLICK_MENU', 'Accordian Menu' );
define( '_SW_AUTO_CLOSE_LABEL', 'Auto Close Open Submenus?' );
define( '_SW_UPGRADE_VERSIONS', 'Current Installed swMenuPro Versions' );
define( '_SW_SELECTED_LANGUAGE_HEADING', 'Current Language File' );
define( '_SW_LANGUAGE_FILES', 'Select New Language File' );
define( '_SW_LANGUAGE_CHANGE_BUTTON', 'Change Language' );
define( '_SW_UPLOAD_LANGUAGE_HEADING', 'Upload New Language File' );
define( '_SW_LANGUAGE_UPLOAD_BUTTON', 'Upload Language File' );
define( '_SW_FILE_PERMISSIONS', 'Current File Permissions' );
define( '_SW_LANGUAGE_SUCCESS', 'Succesfully Added New swMenuPro Language File.' );
define( '_SW_LANGUAGE_FAIL', 'Could not upload language file, please make sure all directories listed below are writable.' );



//Menu Names
define( '_SW_TIGRA_MENU', 'Tigra Menü' );
define( '_SW_TRANS_MENU', 'Trans Menü' );
define( '_SW_MYGOSU_MENU', 'MyGosu Menü' );
define( '_SW_CLICK_MENU', 'Click Menü' );
define( '_SW_TAB_MENU', 'CSS Tab Menü' );
define( '_SW_DYN_TAB_MENU', 'Dynamic Tab Menü' );
define( '_SW_TREE_MENU', 'Tree Menü' );

//Page Names
define( '_SW_MENU_MODULE_MANAGER', 'Menü Modul Manager' );
define( '_SW_MANUAL_CSS_EDITOR', 'CSS Editor' );
define( '_SW_INDIVIDUAL_ITEM_EDITOR', 'Individueller Menüpunkt Editor' );
define( '_SW_MODULE_EDITOR', 'Modul Editor' );
define( '_SW_UPGRADE_FACILITY', 'Upgrade Menü' );


//Common Terms
define( '_SW_WRITABLE', 'Beschreibbar' );
define( '_SW_UNWRITABLE', 'Nicht beschreibbar' );
define( '_SW_YES', 'Ja' );
define( '_SW_NO', 'Nein' );
define( '_SW_HYBRID', 'gemischt' );


//Menu Module Manager Page (list menu modules)
define( '_SW_MENU_MODULES', 'Menü Module' );
define( '_SW_DISPLAY', 'anzeigen' );
define( '_SW_USE_DEFAULT_MODULE', 'Benutze Standard Menü Style' );
define( '_SW_COPY_MODULE', 'Kopiere existierenden Menü Style' );
define( '_SW_CREATE_MODULE', 'Erstelle neues Menümodul' );
define( '_SW_MODULE_NAME', 'Modul Name' );
define( '_SW_SELECT_MENU', 'Wähle Menü System' );
define( '_SW_SELECT_STYLE', 'Wähle Menü Style zum kopieren' );


//Tool Tips
define( '_SW_SELECT_ITEM_TIP', 'Wähle einen Menüeintrag indem du auf seine Bezeichnung klickst.' );
define( '_SW_EDIT_MODULE_TIP', 'Klicke hier um den Style sowie die Einstellung des %s Menümoduls zu bearbeiten' );
define( '_SW_COPY_STYLE_TIP', 'Wähle ein existierendes Menümodul um dessen Styleeinstellungen auf ein neues Modul zu übertragen' );
define( '_SW_EDIT_CSS_TIP', 'Klicke hier um das externe Stylesheet für das %s Menümodul manuell zu bearbeiten' );
define( '_SW_EXPORT_MODULE_TIP', 'Klicke hier um das Stylesheet für das %s Menümodul zu exportieren');
define( '_SW_EDIT_IMAGES_TIP', 'Klicke hier um individuelle Menüeintragsbilder, CCS und Eigenschaft des %s Menümoduls zu bearbeiten' );
define( '_SW_PREVIEW_MODULE_TIP', 'Klicke hier um eine Vorschau des %s Menümoduls in einem PopUp Fenster zu betrachen' );
define( '_SW_DELETE_MODULE_TIP', 'Klicke hier um das %s Menümodul zu löschen' );
define( '_SW_MENU_SYSTEM_INFO_TIP', '<b>Klicke hier</b> um mehr Informationen über die verschiedenen Menüsysteme zu erhalten' );
define( '_SW_MODULE_TIP', '<b>Menü Style:</b> %s<br /><b>QuellMenü:</b> %s<br /><b>Position:</b> %s<br /><b>Zugriff:</b> %s<br /><b>Veröffentlicht:</b> %s');
define( '_SW_CREATE_MENU_TIP', 'Klicke hier um dein eigenes neues Modul zu erstellen.');

define( '_SW_SAVE_TIP', 'Klicke hier um alle Style und Moduländerungen in der Datenbank zu speichern' );
define( '_SW_APPLY_TIP', 'Klicke hier um alle Style und Moduländerungen in die Datenbank zu übernehmen' );
define( '_SW_CANCEL_TIP', 'Klicke hier um alle Änderungen zu verwerfen und zum Modulmanager zurückzukehren' );
define( '_SW_PREVIEW_TIP', 'Klicke hier für eine Vorschau des Menümodul in einem neuen Fenster' );
define( '_SW_EXPORT_TIP', 'Klicke hier um ein externe Stylesheet mit den aktuellen Einstellungen zu exportieren' );

//Buttons text
define( '_SW_SAVE_BUTTON', 'speichern' );
define( '_SW_APPLY_BUTTON', 'anwenden' );
define( '_SW_CANCEL_BUTTON', 'abbrechen' );
define( '_SW_PREVIEW_BUTTON', 'vorschau' );
define( '_SW_EXPORT_BUTTON', 'exportieren' );
define( '_SW_CREATE_BUTTON', 'Jetzt erstellen' );
define( '_SW_EDIT_BUTTON', 'bearbeiten' );
define( '_SW_DELETE_BUTTON', 'löschen' );
define( '_SW_EDITCSS_BUTTON', 'bearbeite CSS' );
define( '_SW_GET_IMAGE_BUTTON', 'lade Bild' );
define( '_SW_UPDATE_CSS_BUTTON', 'Aktualisiere CSS' );
define( '_SW_UPLOAD_BUTTON', 'Datei hochladen' );
define( '_SW_UPDATE_OVER_CSS_BUTTON', 'Aktualisiere die CSS' );


//Internal program links
define( '_SW_UPGRADE_LINK', 'Upgrade/Repariere swMenuPro.' );
define( '_SW_MANAGER_LINK', 'Editiere Menümoduleigenschaften' );
define( '_SW_CSS_LINK', 'Editiere externe CSS Datei' );
define( '_SW_EXPORT_LINK', 'Exportiere eine CSS Datei' );
define( '_SW_RETURN_MANAGER_LINK', 'Kehre zum swMenuPro Modulmanager zurück' );


//Program Notices
define( '_SW_NO_MODULE_NOTICE', 'Du musst auch das Modul installieren, damit swMenuPro richtig funktioniert.' );
define( '_SW_NO_MENU_NOTICE', 'Bitte wähle ein Menüsystem aus der Auswahlliste.' );
define( '_SW_DELETE_MODULE_NOTICE', 'Bist du sicher das du dieses Modul löschen möchtest?' );
define( '_SW_MAKE_MODULE_NOTICE', 'Bitte erstelle ein neues Menümodul mit Hilfe des Assistenten auf der rechten Seite.' );
define( '_SW_UPLOAD_FILE_NOTICE', 'Bitte wähle eine Datei zum hochladen aus.' );
define( '_SW_MENU_CHANGE_NOTICE', 'Du hast die Datenquelle für das Menümodul geändert. Diese Seite kann nicht angezeigt werden, bis du die Einstellungen gespeichert oder sie verworfen hast.' );


//Program Messages
define( '_SW_DELETE_MODULE_MESSAGE', 'Menümodul erfolgreich gelöscht' );
define( '_SW_SAVE_MENU_MESSAGE', 'Einstellungen erfolgreich gespeichert' );
define( '_SW_SAVE_MENU_CSS_MESSAGE', 'Einstellungen gespeichert und externe CSS Datei erfolgreich erstellt' );
define( '_SW_SAVE_CSS_MESSAGE', 'Externe CSS Datei erfolgreich gespeichert' );
define( '_SW_NO_SAVE_MENU_CSS_MESSAGE', 'Externe CSS Datei konnte nicht erstellt werden. Bitte stell sicher das der Ordner modules/mod_swmenupro/styles beschreibbar ist.' );


//////////////////////////
//Upgrade page
/////////////////////////
define( '_SW_OK', 'Alles ist OK' );
define( '_SW_MESSAGES', 'Nachrichten' );
define( '_SW_MODULE_SUCCESS', 'Modul wurde erfolgreich aktualisiert.' );
define( '_SW_MODULE_FAIL', 'Modul konnte nicht aktualisiert werden. Bitte stell sicher das der /modules Ordner beschreibbar ist.' );
define( '_SW_TABLE_UPGRADE', 'Aktualisierte Tabelle %s' );
define( '_SW_TABLE_CREATE', 'Erstellte Tabelle %s' );
define( '_SW_UPDATE_LINKS', 'Aktualisierte AdminMenü Links' );

define( '_SW_MODULE_VERSION', 'Aktuelle swMenuPro Modul Version' );
define( '_SW_COMPONENT_VERSION', 'Aktuelle swMenuPro Komponenten Version' );
define( '_SW_UPLOAD_UPGRADE', 'Lade swMenuPro Upgrade/Release Datei hoch' );
define( '_SW_UPLOAD_UPGRADE_BUTTON', 'Upload & Installiere Datei' );

define( '_SW_COMPONENT_SUCCESS', 'swMenuPro Komponente erfolgreich aktualisiert.' );
define( '_SW_COMPONENT_FAIL', 'Aktualisierung fehlgeschlagen. Bitte stellen Sie sicher das alle unten aufgeführten Verzeichnisse beschreibbar sind.' );
define( '_SW_INVALID_FILE', 'Dies scheint keiner gültige neue swMenuPro upgrade/release Datei zu sein.' );



//////////////////////////
//Item images and CSS page
/////////////////////////
define( '_SW_AUTO_MULTIPLE_LABEL', 'Bearbeite automatisch mehrere Menüeinträge.' );
define( '_SW_AUTO_CSS_CREATOR_LABEL', 'CSS Assistent.' );
define( '_SW_PROPERTIES_LABEL', 'Menüeintrags Eigenschaften' );

//top tabs
define( '_SW_ITEM_PROPERTIES_TAB', 'Eigenschaften<br /> Bilder' );
define( '_SW_ITEM_CSS_TAB', 'Normal / Mouse Over<br /> CSS' );
define( '_SW_MULTIPLE_TAB', 'Bearbeite <br />Mehrere Einträge' );

//general text
define( '_SW_STEP_1', 'Schritt 1' );
define( '_SW_STEP_2', 'Schritt 2' );
define( '_SW_STEP_3', 'Schritt 3' );
define( '_SW_SELECTED_ITEM', 'Ausgewählter Menüeintrag' );
define( '_SW_NONE_SELECTED', 'Nichts ausgewählt' );
define( '_SW_ITEM_PROPERTIES', 'Eintragseigenschaften' );
define( '_SW_SHOW_ITEM', 'Zeige Menüeintrag' );
define( '_SW_SHOW_ITEM_NAME', 'Zeige den Namen des Menüeintrags' );
define( '_SW_IS_LINK', 'Der Eintrag ist ein Link' );
define( '_SW_IMAGE_ALIGN', 'Bild Ausrichtung' );

//Select box text
define( '_SW_CSS_SELECT', 'Wähle eine CSS Einstellung zum bearbeiten aus' );
define( '_SW_COMPLETE_BORDER_SELECT', 'Komplett umrandet' );
define( '_SW_BORDER_TOP_SELECT', 'Rand oben' );
define( '_SW_BORDER_RIGHT_SELECT', 'Rand rechts' );
define( '_SW_BORDER_BOTTOM_SELECT', 'Rand unten' );
define( '_SW_BORDER_LEFT_SELECT', 'Rand links' );
define( '_SW_PADDING_SELECT', 'Padding' );
define( '_SW_MARGIN_SELECT', 'Einzug' );
define( '_SW_BACKGROUND_SELECT', 'Hintergrund' );
define( '_SW_TEXT_SELECT', 'Text' );
define( '_SW_FONT_SELECT', 'Schrift' );
define( '_SW_OFFSET_SELECT', 'Offsets' );
define( '_SW_DIMENSION_SELECT', 'Dimensionen' );
define( '_SW_EFFECT_SELECT', 'Spezialeffekte' );

define( '_SW_AUTO_SELECT', 'Gewählte Menüeinträge' );
define( '_SW_AUTO_ALL_SELECT', 'Alle Menüeinträge' );
define( '_SW_AUTO_TOP_SELECT', 'Top Menüeinträge' );
define( '_SW_AUTO_SUB_SELECT', 'Unter-Menüeinträge' );
define( '_SW_AUTO_FOLDER_SELECT', 'Ordner Menüeinträge' );
define( '_SW_AUTO_DOCUMENT_SELECT', 'Dokumenten Menüeinträge' );

define( '_SW_ATTRIBUTE_SELECT', 'Wähle ein Attribut zum bearbeiten' );
define( '_SW_ATTRIBUTE_IMAGE_SELECT', 'Bild' );
define( '_SW_ATTRIBUTE_OVER_IMAGE_SELECT', 'Mouse Over Bild' );
define( '_SW_ATTRIBUTE_SHOW_NAME_SELECT', 'Zeige Eintragtitel' );
define( '_SW_ATTRIBUTE_DONT_SHOW_NAME_SELECT', 'Zeige Eintragstitel nicht an' );
define( '_SW_ATTRIBUTE_IS_LINK_SELECT', 'Eintrag ist ein Link' );
define( '_SW_ATTRIBUTE_IS_NOT_LINK_SELECT', 'Eintrag ist kein Link' );
define( '_SW_ATTRIBUTE_SHOW_ITEM_SELECT', 'Zeige Menüeintrag' );
define( '_SW_ATTRIBUTE_DONT_SHOW_ITEM_SELECT', 'Zeige Menüeintrag nicht an' );
define( '_SW_ATTRIBUTE_IMAGE_LEFT_SELECT', 'Bild links anordnen' );
define( '_SW_ATTRIBUTE_IMAGE_RIGHT_SELECT', 'Bild rechts anordnen' );
define( '_SW_ATTRIBUTE_CSS_SELECT', 'Normal CSS' );
define( '_SW_ATTRIBUTE_OVER_CSS_SELECT', 'Mouse Over CSS' );


//Extra CSS text
define( '_SW_CSS', 'CSS' );
define( '_SW_CSS_OVER', 'Mouse Over CSS' );
define( '_SW_IMAGE', 'Bild' );
define( '_SW_IMAGE_OVER', 'Mouse Over Bild' );
define( '_SW_PREVIEW', 'Vorschau' );
define( '_SW_IMAGE_URL', 'Bild URL' );
define( '_SW_HSPACE', 'H Abstand' );
define( '_SW_VSPACE', 'V Abstand' );
define( '_SW_REPEAT', 'Wiederholung' );
define( '_SW_TEXT_DECORATION', 'Rand' );
define( '_SW_TEXT_TRANSFORM', 'Transform' );
define( '_SW_TEXT_INDENT', 'Zeileneinzug' );
define( '_SW_WHITE_SPACE', 'Leerzeichen' );
define( '_SW_FONT_STYLE', 'Schriftart' );

//tool tips
define( '_SW_STEP_1_TIP', 'Wähle die Menüeinträge auf die du die Einstellungen anwenden möchtest.' );
define( '_SW_STEP_2_TIP', 'Wähle die Attribute die du ändern möchtest.' );
define( '_SW_STEP_3_TIP', 'Klicke auf anwenden um die ausgewählten Attribute auf die Einträge anzuwenden.' );



//////////////////////////////
//Size Position & Offsets Page
/////////////////////////////
define( '_SW_POSITION_LABEL', 'Menüposition und Orientierung' );
define( '_SW_SIZES_LABEL', 'Menüeintragsgröße' );
define( '_SW_TOP_OFFSETS_LABEL', 'Menü Abstand' );
define( '_SW_SUB_OFFSETS_LABEL', 'Untermenü Abstand' );
define( '_SW_CLICK_DIMENSIONS_LABEL', 'Klicke für Menüdimensionen' );
define( '_SW_ALIGNMENT_LABEL', 'Menüausrichtung' );
define( '_SW_WIDTHS_LABEL', 'Menüeintragsbreite' );
define( '_SW_HEIGHTS_LABEL', 'Menüeintragshöhe' );
define( '_SW_COMPLETE_PADDING_LABEL', 'Komplettes Menüpadding' );

//tree menus
define( '_SW_OTHER_SETTINGS_LABEL', 'Sonstige Einstellungen' );
define( '_SW_TREE_WIDTH_LABEL', 'Menübreite' );
define( '_SW_FOLDER_LINKS', 'Ordner haben Links?' );
define( '_SW_USE_LINES', 'Benutze Linien?' );
define( '_SW_USE_ICONS', 'Benutze Icons?' );

define( '_SW_TOP_MENU', 'Menü' );
define( '_SW_SUB_MENU', 'Untermenü' );
define( '_SW_ALIGNMENT', 'Ausrichtungt' );
define( '_SW_POSITION', 'Position' );
define( '_SW_ORIENTATION', 'Orientierung' );
define( '_SW_ITEM_WIDTH', 'Breite' );
define( '_SW_ITEM_HEIGHT', 'Höhe' );
define( '_SW_TOP_OFFSET', 'Oberer Abstand' );
define( '_SW_LEFT_OFFSET', 'Linker Abstand' );
define( '_SW_LEVEL', 'Level' );
define( '_SW_AUTOSIZE', '(gebe 0 ein für automatische Größe)' );
define( '_SW_TAB_MARGIN', 'Abstand zwischen Tabs' );


//////////////////////
//Fonts & Padding Page
/////////////////////
define( '_SW_FONT_FAMILY_LABEL', 'Schriftart' );
define( '_SW_FONT_SIZE_LABEL', 'Schriftgröße' );
define( '_SW_FONT_ALIGNMENT_LABEL', 'Text Ausrichtung' );
define( '_SW_FONT_WEIGHT_LABEL', 'Schriftstil' );
define( '_SW_PADDING_LABEL', 'Einzug' );


define( '_SW_TOP', 'Oben' );
define( '_SW_RIGHT', 'Rechts' );
define( '_SW_BOTTOM', 'Unten' );
define( '_SW_LEFT', 'Links' );
define( '_SW_FONT_SIZE', 'Schriftgröße' );
define( '_SW_FONT_ALIGNMENT', 'Text Ausrichtung' );
define( '_SW_FONT_WEIGHT', 'Schriftstil'  );
define( '_SW_PADDING', 'Einzug' );
define( '_SW_FONT_TIP', 'Alle Browser zeigen Schriftarten und Größen unterschiedlich an. Die Liste unten zeigt wie dein Browser die möglichen Schriftarten und Größen dargestellt hat.' );

/////////////////////////
//Borders & Effects Page
////////////////////////
define( '_SW_BORDER_WIDTHS_LABEL', 'Randdicke' );
define( '_SW_BORDER_STYLES_LABEL', 'Randstil' );
define( '_SW_SPECIAL_EFFECTS_LABEL', 'Spezialeffekte' );

define( '_SW_OUTSIDE_BORDER', 'Außenseitiger Rand' );
define( '_SW_INSIDE_BORDER', 'Innerer Rand' );
define( '_SW_NORMAL_BORDER', 'Rand' );
define( '_SW_OVER_BORDER', 'Mouse Over Rand' );
define( '_SW_WIDTH', 'Breite' );
define( '_SW_HEIGHT', 'Höhe' );
define( '_SW_DIVIDER', 'Trenner' );
define( '_SW_STYLE', 'Style' );
define( '_SW_DELAY', 'öffnen/Schließen Verzögerung' );
define( '_SW_OPACITY', 'Transparenz' );

///////////////////////////
//Colors & Backgrounds Page
///////////////////////////
define( '_SW_BACKGROUND_IMAGE_LABEL', 'Hintergrund Bilder' );
define( '_SW_BACKGROUND_COLOR_LABEL', 'Hintergrund Farben' );
define( '_SW_FONT_COLOR_LABEL', 'Schriftfarben' );
define( '_SW_BORDER_COLOR_LABEL', 'Randfarben' );

//tab menus
define( '_SW_TAB_ACTIVE', 'Aktiver Reiter' );
define( '_SW_TAB_TOP', 'Menü Reiter' );
define( '_SW_DIVIDER_COLOR', 'Farbe des Trenners' );

//tree menu
define( '_SW_ICONS_LABEL', 'Menü Icons' );
define( '_SW_ICON_TOP', 'Top Icon' );
define( '_SW_ICON_FOLDER', 'Ordner Icon' );
define( '_SW_ICON_FOLDER_OPEN', 'offener Ordner Icon' );
define( '_SW_ICON_DOCUMENT', 'Dokumenten Icon' );


define( '_SW_BACKGROUND', 'Hintergrund' );
define( '_SW_OVER_BACKGROUND', 'Mouse Over Hintergrund' );
define( '_SW_COLOR', 'Farbe' );
define( '_SW_OVER_COLOR', 'Mouse Over Farbe' );
define( '_SW_FONT', 'Schriftfarbe' );
define( '_SW_OVER_FONT', 'Mouse Over Schriftfarbe' );
define( '_SW_OUTSIDE_BORDER_COLOR', 'Äußere Randfarbe' );
define( '_SW_INSIDE_BORDER_COLOR', 'Innere Randfarbe' );
define( '_SW_NORMAL_BORDER_COLOR', 'Randfarbe' );
define( '_SW_OVER_BORDER_COLOR', 'Mouse Over Randfarbe' );
define( '_SW_GET', 'hole' );
define( '_SW_COLOR_TIP', 'Wähle eine Farbe vom Farbrad und klicke danach auf %s neben der Farbauswahl Box um die Farbe anzuwenden.');
define( '_SW_PRESENT_COLOR', 'aktuelle Farbe' );
define( '_SW_SELECTED_COLOR', 'ausgewählte Farbe' );


///////////////////////////
//Menu Module Settings Page
///////////////////////////
define( '_SW_MENU_SOURCE_LABEL', 'Menüquelle Einstellungen' );
define( '_SW_STYLE_SHEET_LABEL', 'Stylesheet Einstellungen' );
define( '_SW_AUTO_ITEM_LABEL', 'Automatische Menüeinträge' );
define( '_SW_CACHE_LABEL', 'Cache Einstellungen' );
define( '_SW_GENERAL_LABEL', 'Allgemeine Moduleinstellungen' );
define( '_SW_POSITION_ACCESS_LABEL', 'Position & Zugriff' );
define( '_SW_PAGES_LABEL', 'Zeige Menümodul auf folgenden Seiten' );
define( '_SW_CONDITIONS_LABEL', 'Bedingungen' );

//Select box text
define( '_SW_CSS_DYNAMIC_SELECT', 'Schreibe Stylesheet direkt in die Seite' );
define( '_SW_CSS_LINK_SELECT', 'Linke auf ein externes Stylesheet' );
define( '_SW_CSS_IMPORT_SELECT', 'Importiere ein externes Stylesheet' );
define( '_SW_CSS_NONE_SELECT', 'Verlinke kein Stylesheet' );

define( '_SW_SOURCE_CONTENT_SELECT', 'Nur Inhalte benutzen' );
define( '_SW_SOURCE_EXISTING_SELECT', 'existierendes Menü wählen');

define( '_SW_SHOW_TABLES_SELECT', 'Darstellung als Tabelle' );
define( '_SW_SHOW_BLOGS_SELECT', 'Darstellung als Blog' );

define( '_SW_10SECOND_SELECT', '10 Sekunden' );
define( '_SW_1MINUTE_SELECT', '1 Minute' );
define( '_SW_30MINUTE_SELECT', '30 Minuten' );
define( '_SW_1HOUR_SELECT', '1 Stunde' );
define( '_SW_6HOUR_SELECT', '6 Stunden' );
define( '_SW_12HOUR_SELECT', '12 Stunden' );
define( '_SW_1DAY_SELECT', '1 Tag' );
define( '_SW_3DAY_SELECT', '3 Tage' );
define( '_SW_1WEEK_SELECT', '1 Woche' );

//top tabs text
define( '_SW_MODULE_SETTINGS_TAB', 'Menümodul Einstellungen' );
define( '_SW_SIZE_OFFSETS_TAB', 'Größe, Position & Abstände' );
define( '_SW_COLOR_BACKGROUNDS_TAB', 'Farbe & Hintergründe' );
define( '_SW_FONTS_PADDING_TAB', 'Schriftart & Einzug' );
define( '_SW_BORDERS_EFFECTS_TAB', 'Ränder & Effekte' );
define( '_SW_IMAGES_CSS_TAB', 'Eintragsbilder & CSS' );
define( '_SW_TREE_SIZE_TAB', 'Größe & andere Einstellungen' );

//general text
define( '_SW_MENU_SOURCE', 'Menüquelle' );
define( '_SW_PARENT', 'Einstiegspunkt' );
define( '_SW_STYLE_SHEET', 'Lade Stylesheet' );
define( '_SW_CLASS_SFX', 'Modul Klassen Suffix' );
define( '_SW_HYBRID_MENU', 'Hybrid Menü' );
define( '_SW_TABLES_BLOGS', 'Verwende Tabellen/Blogs' );
define( '_SW_CACHE_ITEMS', 'Cache Menüeinträge' );
define( '_SW_CACHE_REFRESH', 'Cache Aktualisierungszeit' );
define( '_SW_SHOW_NAME', 'Zeige Modulnamen' );
define( '_SW_PUBLISHED', 'Veröffentlicht');
define( '_SW_ACTIVE_MENU', 'Aktives Menü' );
define( '_SW_MAX_LEVELS', 'Maximale Tiefe' );
define( '_SW_PARENT_LEVEL', 'Ausgangstiefe' );
define( '_SW_SELECT_HACK', 'Select Box Hack' );
define( '_SW_SUB_INDICATOR', 'Untermenü Symbol' );
define( '_SW_SHOW_SHADOW', 'Zeige Schatten' );
define( '_SW_MODULE_POSITION', 'Modul Position' );
define( '_SW_MODULE_ORDER', 'Modul Reihenfolge' );
define( '_SW_ACCESS_LEVEL', 'Zugriffslevel' );
define( '_SW_TEMPLATE', 'Template' );
define( '_SW_LANGUAGE', 'Sprache' );
define( '_SW_COMPONENT', 'Komponente' );

//tool tips
define( '_SW_MENU_SOURCE_TIP', 'Wählen ein gültiges Menü als Quelle für die Menüeinträge dieses Menüs aus.' );
define( '_SW_PARENT_TIP', 'Wählen einen Einstiegspunkt um eine Teilmenge des Menüs anzuzeigen. Wähle TOP um alle Einträge anzuzeigen.' );
define( '_SW_STYLE_SHEET_TIP', '<b>In die Seite:</b> schreibt das Stylesheet direkt in das Dokument, welches das Modul aufruft.<br /><b>Extern verlinken: </b>linkt auf ein externes Stylesheet welches exportiert werden muss.<br /><b>´Nicht verlinken: </b> kopieren sie ihren eigenen Link zum externen Stylesheet in den Head Bereich ihres Templates. Das Menümodul wird dann komplett validiert.' );
define( '_SW_CLASS_SFX_TIP', 'Suffix für die CSS Klassen des Moduls. Kann verwendet werden um Konflikte zu vermeiden.' );
define( '_SW_HYBRID_MENU_TIP', 'Automatisches Anhängen von Inhaltsseiten an diese Menü. Die Einträge müssen vom Typ Section/Category sein.' );
define( '_SW_TABLES_BLOGS_TIP', 'Zeige automatisch erstellte Einträge als Tabellen oder Blogs.' );
define( '_SW_CACHE_ITEMS_TIP', 'Benutze einen Dateicache um die Menüeinträge zu cachen und die Geschwindigkeit zu verbessern. Besonders nützlich bei Hybriden Menüs mit vielen automatisch erstellten Einträge (viele SQL Statements). Diese werden nurnoch einmal innerhalb des Cache Intevalls geladen.' );
define( '_SW_CACHE_REFRESH_TIP', 'Zeitintervall nachdem sich der Cache für die Menüeinträge aktualisiert.' );
define( '_SW_SHOW_NAME_TIP', 'Zeigt den Menümodulnamen.' );
define( '_SW_PUBLISHED_TIP', 'Veröffentlich oder unveröffentlicht das Menümodul.');
define( '_SW_ACTIVE_MENU_TIP', 'Behält den momentan ausgewählten Eintrag als aktiven Eintrag. So kann man erkennen, welche Seite man gerade besucht.' );
define( '_SW_MAX_LEVELS_TIP', 'Maximale Anzahl an Ebenen des Quellmenüs, welche dargestellt wird. Wähle 0 um alle Ebenen anzuzeigen.' );
define( '_SW_PARENT_LEVEL_TIP', 'Erweiterte Einstellung welche das Quellmenü bis zu einem gewissen Level zurückverfolgt. Normalerweise auf 0 gestellt.' );
define( '_SW_SELECT_HACK_TIP', 'Verwende einen Hack um die Darstellung von Untermenüs über Auswahlboxen  im Internet Explorer zu ermöglichen.' );
define( '_SW_SUB_INDICATOR_TIP', 'Zeigt einen kleinen Pfeil um anzuzeigen, das ein Menüeintrag noch weitere Untereinträge besitzt.' );
define( '_SW_SHOW_SHADOW_TIP', 'Zeigt einen Schatten um Untermenüs.' );
define( '_SW_MODULE_POSITION_TIP', 'Position des Menümoduls im Template.' );
define( '_SW_MODULE_ORDER_TIP', 'Reihenfolge des Menümoduls in der Templateposition.' );
define( '_SW_ACCESS_LEVEL_TIP', 'Zugriffslevel für das Menümodul.' );
define( '_SW_TEMPLATE_TIP', 'Das Menümodul wird nur bei folgenden Templates dargestellt.' );
define( '_SW_LANGUAGE_TIP', 'Das Menümodul wird nur in folgenden Sprachen dargestellt.' );
define( '_SW_COMPONENT_TIP', 'Das Menümodul wird nur bei folgenden Komponenten dargestellt.' );
define( '_SW_PAGES_TIP', 'Wähle die Seiten: <i>(halte STRG gedrückt während du mit der linken Maustaste Einträge auswählst um mehrere zu wählen.)</i>' );


?>