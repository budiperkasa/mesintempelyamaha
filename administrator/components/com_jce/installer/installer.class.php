...
class mosInstaller {
	...
	*/
	function findInstallFile() {
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, 'ERROR: Could not find a JCE 1.1.x XML setup file in the package.' );
			return false;
		} else {
	...
	*/
	function readInstallFile() {
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, 'File :"' . $this->installFilename() . '" is not a valid JCE installation file or is the wrong version.' );
			return false;
		}
		if ($root->getAttribute('version') != '1.1.0') {
			$this->setError( 1, 'Incorrect Plugin version.' );
			return false;
		}
